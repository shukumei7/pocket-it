...
module.exports = function createScriptsRouter() {
  ...
  router.post('/', (req, res) => {
    const { name, description, script_content, category, requires_elevation, timeout_seconds } = req.body;
    ...
    try {
      ...
      ).run(name, description || null, script_content, category || 'general', requires_elevation || 0, timeout_seconds || 60);
  ...
  router.patch('/:id', (req, res) => {
    const { id } = req.params;
    const allowed = ['name', 'description', 'script_content', 'category', 'requires_elevation', 'timeout_seconds'];
    const updates = [];
    const values = [];
